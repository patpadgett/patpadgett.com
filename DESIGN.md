...
Sections stack with `clamp(48px,7vw,90px)` top padding and open with a section tag (PP-001…PP-007; About Patrick is PP-001) beside the display heading and one lede line. Page order = channel order: About Patrick (VHS sleeve + bio), Work (Mac + log), Music (cassette), Octavitin (pitch | book | card, `0.75fr / 1.2fr / 0.7fr`), Booking records (Polaroid stack | printer), the coffee table (magazine fan), Madballs (3×3), Contact (five GPK cards under the wax pack). About Patrick (PP-001) leads; the sections carry PP-002…PP-007 in that order.
